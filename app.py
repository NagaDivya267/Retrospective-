...
def save_mood_data_to_google_sheet(team_name: str, sprint_name: str, average_mood: float, mood_status: str) -> None:
    ...
    existing_header = [header.strip() for header in get_sheet_row_values(MOOD_WORKSHEET_NAME, 1, rows=500, cols=10)]

    if not existing_header:
        sheet.append_row(MOOD_SHEET_COLUMNS)
        existing_header = MOOD_SHEET_COLUMNS.copy()
    else:
        # Expand legacy schema in-place without deleting existing mood history.
        for column_index, header in enumerate(MOOD_SHEET_COLUMNS, start=1):
    ...
    normalized_sprint = sprint_name.strip()
    matched_row_number = None
    matched_record = None

    for row_index, record in enumerate(records, start=2):
        record_team = str(record.get("Team Name", "")).strip()
        record_sprint = str(record.get("PI or Sprint Name", "")).strip()
        if record_team.casefold() == normalized_team.casefold() and record_sprint.casefold() == normalized_sprint.casefold():
            matched_row_number = row_index
    ...
    if matched_row_number is None:
        ...
        try:
            existing_score = float(str((matched_record or {}).get("Average Mood Score", "0") or "0").strip())
        except (TypeError, ValueError):
            existing_score = 0.0

        try:
            existing_count = int(str((matched_record or {}).get("Responses Count", "1") or "1").strip())
        except (TypeError, ValueError):
            existing_count = 1

        existing_count = max(existing_count, 1)
        new_count = existing_count + 1
        new_average = ((existing_score * existing_count) + float(average_mood)) / new_count
        new_status = get_mood_status(new_average)

        sheet.update(f"A{matched_row_number}:E{matched_row_number}", [[
            normalized_team,
            normalized_sprint,
            f"{new_average:.2f}",
            new_status,
...
def render_interactive_fishbone(
    ...
    html = f"""
        ...
        <div class="fb-container" id="fb">
            ...
            <div class="footer">Use the vote section below to record team votes.</div>
        </div>
...
def get_fishbone_priority_from_votes(votes: int) -> str:
    if votes >= 4:
        return "High"
    if votes >= 2:
        return "Medium"
...
def get_top_voted_fishbone_causes(
    fishbone_data: dict[str, list[FishboneCause]],
    limit: int = 3,
) -> list[dict[str, str | int]]:
    ranked_causes: list[dict[str, str | int]] = []
    for category in FISHBONE_CATEGORIES:
        for cause in fishbone_data.get(category, []):
            votes = int(cause.get("votes", 0))
            if votes <= 0:
                continue
            ranked_causes.append(
                {
                    "id": str(cause.get("id", "")),
                    "text": str(cause.get("text", "")).strip(),
                    "category": category,
                    "votes": votes,
                }
            )
    ...
    ranked_causes.sort(
        key=lambda item: (
            -int(item["votes"]),
            FISHBONE_CATEGORIES.index(str(item["category"])),
            str(item["text"]).casefold(),
        )
    )
    return ranked_causes[: max(limit, 1)]


def sync_fishbone_actions_to_tracker(fishbone_data: dict[str, list[FishboneCause]]) -> int:
    rows: list[tuple[str, str, str]] = []
    for category in FISHBONE_CATEGORIES:
        for cause in fishbone_data.get(category, []):
            action_text = f"[Fishbone] {category}: {cause['text']}"
            action_priority = get_fishbone_priority_from_votes(int(cause.get("votes", 0)))
            rows.append((action_text, action_priority, TEAM_ACTION_SOURCE))
    ...
    saved_count = 0
    for action_text, action_priority, source in rows:
        action_key = action_text.strip().casefold()
        if not action_key or action_key in existing_actions:
            continue
        action_sheet.append_row([action_text, action_priority, source])
        existing_actions.add(action_key)
        saved_count += 1

    if saved_count > 0:
        clear_google_sheet_read_cache()
        mark_sync_event("Synced fishbone actions")
...
"""
    ...
    st.session_state["fishbone_top_root_causes"] = top_root_causes
    if not top_root_causes:
        st.info("No voted root causes yet. Votes decide top root causes.")
    else:
        for rank, item in enumerate(top_root_causes, start=1):
            st.success(f"{rank}. {item['text']} ({item['category']}) - 👍 {item['votes']}")
        st.caption("These top voted root causes are available in the Dashboard discussion section.")
    ...
    st.caption("Use the vote buttons below to collect team votes.")
